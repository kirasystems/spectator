...
const Document = (props: DocumentProps) => {
  ...
  const nextDocumentId = React.useMemo(() => {
    ...
    return nextDocumentIndex >= 0 && nextDocumentIndex < documents.length
      ? documents[nextDocumentIndex].id
      : null;
  }, [id, documents]);
  ...
  const previousDocumentId = React.useMemo(() => {
    ...
    return previousDocumentIndex >= 0 &&
      previousDocumentIndex < documents.length
      ? documents[previousDocumentIndex].id
      : null;
  }, [id, documents]);

  const handleAnnotationCreate = React.useCallback(
    (annotation: any) => {
      annotation.topicId = topics.find(
        (topic) => topic.topic === annotation.topic
      )?.id;
  ...
  const handleAnnotationDelete = React.useCallback(
    (annotation: any) => {
      fetch(
        "/document/" + document?.id + "/annotation/" + annotation.annotationId,
        {
          method: "delete",
        }
      )
    ...
    <Box height="100%" width="100%" margin={"0"}>
      {document && (
        <DocumentViewer
          ...
          onNextDocument={() =>
            nextDocumentId && history.push("/document/" + nextDocumentId)
          }
          onPreviousDocument={() =>
            previousDocumentId &&
            history.push("/document/" + previousDocumentId)
          }
          ...
          muiClassGenerator={classGenerator}
        />
      )}
    </Box>
  );
